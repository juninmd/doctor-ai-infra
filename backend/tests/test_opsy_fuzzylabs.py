...
def test_supervisor_opsy_routing():
    ...
    state = {
        "messages": [
            HumanMessage(
                content="Run the Opsy workflow to backup and ticket failing pods.")]}

    fake_llm = _create_mock_llm(
        "Opsy_Specialist",
        "Request requires executing the Opsy workflow, routing to Opsy_Specialist.")
...
def test_supervisor_fuzzylabs_routing():
    ...
    state = {
        "messages": [
            HumanMessage(
                content="Use the FuzzyLabs workflow to diagnose the frontend service.")]}

    fake_llm = _create_mock_llm(
        "FuzzyLabs_Specialist",
        "Request involves the FuzzyLabs workflow, routing to FuzzyLabs_Specialist.")
...
def test_opsy_backup_and_ticket_failing_pods_llm_fallback(
        mock_get_llm, mock_get_sdk, mock_create_issue, mock_list_pods):
    """Verifies that the Opsy tool falls back to the standard LLM if Gemini SDK fails."""

    # Needs to mock the import from inside the function as it uses local
    # imports
    with patch("app.tools.list_k8s_pods") as mock_local_list_pods, \
        ...
        result = opsy_backup_and_ticket_failing_pods.invoke(
            {"namespace": "default", "project": "TEST"})
