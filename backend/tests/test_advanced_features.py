...
def test_generate_postmortem_indexing(
        db_session,
        mock_google_sdk,
        mock_rag,
        mock_llm):
...
def test_generate_runbook_from_incident(
        db_session,
        mock_google_sdk,
        mock_rag,
        mock_llm):
    ...
    pm = PostMortem(
        incident_id="inc-456",
        content="Root Cause: Deadlock. Action: Restart DB.")
    db_session.add(inc)
    db_session.add(pm)
    db_session.commit()

    # 2. Call generate_runbook_from_incident
    result = generate_runbook_from_incident.invoke(
        {"incident_id": "inc-456", "runbook_name": "auto_db_restart"})
...
def test_execute_runbook_manual(db_session):
    ...
    result = execute_runbook.invoke(
        {"runbook_name": "manual_check", "target_service": "test-service"})
