...
def test_supervisor_routes_to_planner(mock_llm):
    ...
    mock_decision = RouterSchema(
        reasoning="Complex issue, need a plan.",
        next_agent="Planner_Specialist")
    ...
    with patch("app.graph.ChatPromptTemplate") as MockPrompt:
        ...
        state = {
            "messages": [
                HumanMessage(
                    content="I have a weird latency issue across multiple services. I don't know where to start.")]}
