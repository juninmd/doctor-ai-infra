...
def test_create_incident(db_session):
    ...
    result = create_incident.invoke(
        {"title": "Database Slow", "severity": "SEV-2", "description": "High latency on primary DB"})
...
def test_incident_workflow(
        ...
        mock_google_sdk):
    # 1. Create
    res = create_incident.invoke(
        {"title": "API Down", "severity": "SEV-1", "description": "500 errors on /api/v1"})
    # Extract ID (format: "Incident created successfully. ID: XXXXXXXX")
    inc_id = res.split("ID: ")[1].strip()

    # 2. Log Event
    log_res = log_incident_event.invoke(
        {
            "incident_id": inc_id,
    ...
    # 3. Update Status
    upd_res = update_incident_status.invoke(
        {"incident_id": inc_id, "status": "INVESTIGATING", "update_message": "Checking logs"})
    ...
    plan = generate_remediation_plan.invoke(
        {"incident_context": "Connection refused to Postgres"})
...
def test_list_incidents(db_session):
    create_incident.invoke(
        {"title": "Inc 1", "severity": "SEV-3", "description": "Minor issue"})
    create_incident.invoke(
        {"title": "Inc 2", "severity": "SEV-1", "description": "Major issue"})
