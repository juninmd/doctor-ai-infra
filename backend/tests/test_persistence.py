...
async def test_persistence_interrupt_resume():
    ...
    # In case it's coerced to RunnableLambda and called directly
    mock_chain.side_effect = decisions

    # mock_llm.with_structured_output returns the chain
    mock_llm.with_structured_output.return_value = mock_chain

    # Use patch context only for get_llm, as reloading will use real
    # ChatPromptTemplate
    with patch("app.llm.get_llm", return_value=mock_llm):
        ...
        inputs = {
            "messages": [
                HumanMessage(
                    content="Please run the cleanup runbook")]}
        ...
        assert state.next == (
            'Automation_Specialist',), f"Graph should be paused at Automation_Specialist, got {
            state.next}"
        ...
        # we update the state as if the agent ran, which is exactly how 'deny'
        # logic works too.
        ...
        events = []
        # Resume. Since we updated state as Automation_Specialist, it should
