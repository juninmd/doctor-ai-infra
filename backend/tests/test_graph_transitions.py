...
def test_supervisor_routes_to_k8s_specialist():
    ...
    state = {
        "messages": [
            HumanMessage(
                content="My pod is crashing, can you analyze the logs?")]}
    ...
    with patch("app.graph.llm") as mock_llm:
        ...
        # We need to test the fallback path as well, so if
        # with_structured_output raises, we catch it
        ...
        with patch("app.graph.ChatPromptTemplate.from_messages") as mock_prompt:
            ...
            mock_chain_fallback.invoke.return_value = {
                "next_agent": "K8s_Specialist", "reasoning": "testing"}
...
def test_supervisor_routes_to_gcp_specialist():
    ...
    state = {
        "messages": [
            HumanMessage(
                content="What is the estimated cost of our GCP infrastructure?")]}
    ...
    with patch("app.graph.llm") as mock_llm:
        ...
        with patch("app.graph.ChatPromptTemplate.from_messages") as mock_prompt:
            mock_chain_fallback = mock_prompt.return_value.partial.return_value.__or__.return_value.__or__.return_value
            mock_chain_fallback.invoke.return_value = {
                "next_agent": "GCP_Specialist", "reasoning": "testing"}
            result = supervisor_node(state)
...
def test_supervisor_routes_to_azion_specialist():
    ...
    state = {
        "messages": [
            HumanMessage(
                content="Clear the Azion CDN cache for the new deployment")]}
    ...
    with patch("app.graph.llm") as mock_llm:
        ...
        with patch("app.graph.ChatPromptTemplate.from_messages") as mock_prompt:
            mock_chain_fallback = mock_prompt.return_value.partial.return_value.__or__.return_value.__or__.return_value
            mock_chain_fallback.invoke.return_value = {
                "next_agent": "Azion_Specialist", "reasoning": "testing"}
            result = supervisor_node(state)
...
def test_supervisor_routes_to_finish():
    ...
    with patch("app.graph.llm") as mock_llm:
        ...
        with patch("app.graph.ChatPromptTemplate.from_messages") as mock_prompt:
            mock_chain_fallback = mock_prompt.return_value.partial.return_value.__or__.return_value.__or__.return_value
            mock_chain_fallback.invoke.return_value = {
                "next_agent": "FINISH", "reasoning": "testing"}
            result = supervisor_node(state)
