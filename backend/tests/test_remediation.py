...
class TestRemediation(unittest.TestCase):
    ...
    def test_generate_remediation_plan_sdk(self, mock_get_client):
        ...
        result = generate_remediation_plan.invoke(
            {"incident_context": "Pod crashing with OOM"})

        # Assertions
        # Since we mocked the SDK, we expect the output to contain the mocked text
        # But we need to see what the tool actually returns.
        # The tool implementation: return f"**Generated Remediation Plan
        # (Gemini):**\n\n{response.text}"
    ...
    def test_generate_remediation_plan_fallback(
            self, mock_get_llm, mock_get_client):
