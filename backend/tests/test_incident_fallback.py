...
def test_generate_remediation_plan_fallback():
    ...
    # Use patch.object on the imported module directly to avoid resolution
    # issues
    with patch.object(incident, "get_google_sdk_client") as mock_get_sdk, \
        ...
        mock_llm_instance.invoke.return_value = MagicMock(
            content="Standard Remediation Plan")
        mock_get_llm.return_value = mock_llm_instance

        # Execute
        result = incident.generate_remediation_plan.invoke(
            {"incident_context": "Service Down"})
