...
def test_rag_initialization(db_session, mock_rag_engine):
    ...
    r1 = Runbook(
        name="restart-db",
        description="How to restart DB",
        implementation_key="restart_db_func")
    db_session.add(s1)
    db_session.add(r1)
    db_session.commit()

    # Verify data is in DB
    assert db_session.query(Service).filter(
        Service.name == "service-a").count() == 1
    assert db_session.query(Runbook).filter(
        Runbook.name == "restart-db").count() == 1
    ...
    # This ensures we use the test DB and test Mock RAG, regardless of import
    # aliasing
    ...
    # Also verify we are NOT seeing the polluted data (optional, but good for
    # sanity)
