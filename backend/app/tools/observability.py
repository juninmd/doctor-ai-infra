...
def scan_infrastructure() -> str:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        try:
            ...
            if "Error" in k8s_res and not "Running" in k8s_res:
                report.append(f"- Kubernetes: 🔴 Issue Detected")
                health_data["k8s"] = {"status": "critical", "msg": k8s_res[:50]}
            else:
                report.append(f"- Kubernetes: 🟢 Active")
                health_data["k8s"] = {"status": "healthy", "msg": "Active"}
        except Exception as e:
            report.append(f"- Kubernetes: 🔴 Failed")
        ...
        try:
            ...
            if "No active alerts" in dd_res:
                 report.append(f"- Datadog: 🟢 No alerts")
                 health_data["datadog"] = {"status": "healthy", "msg": "No alerts"}
            else:
                report.append(f"- Datadog: 🟠 Alertas Ativos")
                health_data["datadog"] = {"status": "warning", "msg": "Alerts active"}
        except Exception as e:
             report.append(f"- Datadog: 🔴 Failed")
        ...
        try:
            ...
            if "🟢" in tr_res:
                 report.append(f"- Traefik: 🟢 OK")
                 health_data["traefik"] = {"status": "healthy", "msg": "OK"}
            else:
                 report.append(f"- Traefik: 🔴 Issue")
                 health_data["traefik"] = {"status": "critical", "msg": "Issue detected"}
        except Exception as e:
    ...
    try:
        if client:
            response = client.models.generate_content(model="gemini-1.5-flash", contents=prompt_msgs)
        ...
        ai_summary = f"System Normal (AI analysis failed)"
