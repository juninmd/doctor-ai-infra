...
def check_traefik_health() -> str:
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            return f"🟢 Traefik Active. Providers: {
                len(
                    data.get(
                        'providers',
                        []))}, HTTP Services: {
        ...
        return f"🟡 Traefik reachable but returned {
            resp.status_code} at {api_url}"
        ...
        try:
            ...
            if pods.items:
                p = pods.items[0]
                return f"🟢 Traefik Pod '{
                    p.metadata.name}' is {
                    p.status.phase} in {
                    p.metadata.namespace}. (API Unreachable: {e})"
            ...
            if rule.http:
                for path in rule.http.paths:
                    ...
                    try:
                        ...
                        ep = v1_core.read_namespaced_endpoints(
                            svc_name, namespace)
                        if not ep.subsets:
                            report.append(
                                f"  ⚠️ Warning: No endpoints found for service '{svc_name}' (Pod mismatch?).")
                        else:
                            ready = sum(len(s.addresses or [])
                                        for s in ep.subsets)
                            report.append(
                                f"  🟢 {ready} healthy endpoints ready.")
                    except BaseException:
        ...
        if ing.spec.tls:
            for t in ing.spec.tls:
                ...
                try:
                    ...
                    report.append(
                        f"- TLS Secret '{secret_name}': 🔴 MISSING (SSL will fail)")
