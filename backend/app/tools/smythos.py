...
@tool
def smythos_unified_resource_manager(
        action: str,
        resource_type: str,
        resource_name: str,
    ...
    try:
        ...
        if resource_type.upper() == "LLM":
            ...
            if action.lower() == "execute":
                llm = get_llm()
                response = llm.invoke(
                    payload if payload else f"Test prompt for {resource_name}")
                return f"[SmythOS LLM Connector] Response from {resource_name}: {
                    response.content}"
            else:
            ...
            if action.lower() == "read":
                results = rag_engine.search(
                    payload if payload else "test", k=1)
                docs = [doc.page_content for doc in results] if results else [
                    "No results"]
