...
- **Frontend**: Uses `vite` instead of Next.js. Migrated to `pnpm` for package management.
- **Testing**: Backend tests use `pytest`.
- **CI/CD**: GitHub Actions runner set to `ubuntu-22.04` to ensure availability. `semantic-release` configured for `main` branch.
